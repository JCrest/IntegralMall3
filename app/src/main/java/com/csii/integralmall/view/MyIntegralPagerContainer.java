 ...
 * 带有动态效果的viewpager
 */
public class MyIntegralPagerContainer extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    public MyIntegralPagerContainer(Context context) {
        super(context);
        init();
    }

    public MyIntegralPagerContainer(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public MyIntegralPagerContainer(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onFinishInflate() {
        try {
            ...
            throw new IllegalStateException("The root child of MyInlegralPagerContainer must be a ViewPager");
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (stayedWithinClickDistance && Math.abs(event.getX() - startX) > com.csii.integralmall.utils.Utils.convertDpToPixel(10, getContext())) {
